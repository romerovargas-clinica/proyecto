...
<head>
  ...
  <script type='text/javascript'>
    function submitLink($funcNum, $dir, $file) {
      window.opener.CKEDITOR.tools.callFunction(1, "images/" + $dir + "/" + $file);
      window.close();
    }
  </script>
</head>
  ...
  <div class="d-flex bd-highlight">
    <div class="p-2 w-100 row row-cols-lg-auto g-3 align-items-center">
      <?php if (isset($adm_pag) && $adm_pag != "chat") : ?>
        <div class="col-12">
          <button type="button" class="page-link btn-sm bg-primary text-white" onclick="window.location='filebrowser2.php?page=<?= ($page) ?>&AddNew';"><?= __('btn_Add', $lang) ?></button>
        </div>
      <?php endif; ?>
      <div class="col-12">
    ...
    <nav aria-label="navigation flex-shrink-1">
      <ul class="pagination justify-content-end">
        <?php if ($total_pages >= 1) {
          if ($page != 1) { ?>
            <li class="page-item"><a class="page-link" href="filebrowser2.php?page=<?= ($page - 1) ?>">&laquo;</a>
            </li>
            <?php }
          ...
          for ($i = 1; $i <= $total_pages; $i++) {
            ...
            <?php } else { ?>
              <li class="page-item"><a class="page-link" href="filebrowser2.php?page=<?= $i ?>"><?= $i ?></a></li>
            <?php }
          }

          if ($page != $total_pages) { ?>
            <li class="page-item"><a class="page-link" href="filebrowser2.php?page=<?= $page + 1 ?>">&raquo;</a>
            </li>
        <?php }
        } ?>
      </ul>
