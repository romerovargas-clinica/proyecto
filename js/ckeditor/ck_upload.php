...
//    print_r($_FILES);    
    define('F_NAME', basename($_FILES['upload']['name']));
//    echo F_NAME;
//    die();
...
//    die();
    ...
    // File upload path    
    $f_name = setFName($_SERVER['DOCUMENT_ROOT'] .'/'. $upload_dir, $sepext[0], ".$type", 0); 
    //$f_name = $_SERVER['DOCUMENT_ROOT'] .'/'. $upload_dir.'/'.$F_NAME;
    var_dump($f_name);
    ...
    if($re == ''){        
        if(move_uploaded_file($_FILES['upload']['tmp_name'], $uploadpath)) { 
            ...
            if(in_array($type, $imgset['type'])){
                ...
                $re = "<script>var cke_ob = window.parent.CKEDITOR; for(var ckid in cke_ob.instances) { if(cke_ob.instances[ckid].focusManager.hasFocus) break;} cke_ob.instances[ckid].insertHtml(\' \', \'unfiltered_html\'); alert(\"". $msg ."\"); var dialog = cke_ob.dialog.getCurrent();dialog.hide();</script>"; 
            }
        }else{
